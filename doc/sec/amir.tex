...
%	Side note: note that $p_i$ is the summation over all possible mutations where $i$ occured, i.e., there are valid (reachable from $r$) nodes in the $\cS$ and also $i \in \cS$. 
	...
	\be 
	\label{condprob} 
	\forall (u, v) \in \cE': \pr(Z_v = b| Z_u = a) = \frac{A_{(u,v)}(a, b) }{\sum_{a \in \{0, 1\}} A_{(u,v)}(a, b) }
	\\ \nr 
	\forall v \in \cV: \pr(X_v = c| Z_v = a) = \frac{B_{v}(c, a) }{\sum_{c \in \{0, 1\}} B_{v}(c, a) }
	\ee 
	...
	\begin{algorithm}[t]
		...
		\label{alg4}
		\begin{algorithmic}[1]
			\STATE {\bfseries input:} Input binary data matrix $\X \in \reals^{n \times p}$, a number of iterations $i$
			\STATE {\bfseries output:} $\cT^{(i)} = (T^{(i)}, \Theta^{(i)})$ where $T^{(i)} = (\cV, \cE^{(i)}, r^{(i)})$
			
			\STATE Initialize $\cT^{(0)} = (T^{(0)}, \Theta^{(0)})$.
			
			\COMMENT{Iterations of the EM algorithm.}
			\FOR{$t = 0$ to $i$}
